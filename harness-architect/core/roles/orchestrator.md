...
**코드 수정·테스트 작성·마이그레이션 실행은 하지 않는다.** 편집 능력이 이 역할에서 빠져 있다
(하네스별 강제 수준은 `references/catalog.md` 의 표를 본다).
...
5. **tier 를 항상 명시한다**: dispatch 시 생략하면 세션의 가장 비싼 모델을 상속한다.
   tier→실제 모델 매핑은 `references/adapters/<하네스>.md` 의 `tier_map` 이 정한다.
...
- [ ] 모든 dispatch 에 tier(또는 그 하네스의 모델 인자)를 명시했다
