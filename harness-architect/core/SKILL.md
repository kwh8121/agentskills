...
> 설치본이 있는 프로젝트 루트를 작업 디렉터리로 실행되어야 한다 (모든 경로가 상대 경로다).
...
## Phase −2 — 하네스 해석 (모든 것보다 먼저, 한 번만)

이 스킬은 Claude Code · Codex · OpenCode 에서 동작한다. **하네스마다 다른 것은 이 문서가
정하지 않는다** — dispatch 문법, 모델 이름, 스킬 호출 방법, 가드 배선, worktree 제약은
전부 어댑터 문서가 정의한다.

```
Bash: bash <설치본>/scripts/detect-harness.sh
```

출력의 `skill_dir` 이 아래 모든 `${skill_dir}` 을 대체하고, `adapter` 가 이번 세션에서
읽을 어댑터 문서다. **그 문서 하나만 읽는다** (`references/adapters/README.md` 는 키 목록).
어댑터에서 가져오는 값:

| 키 | 이 문서에서 쓰는 곳 |
|---|---|
| `skill_dir` | 모든 스크립트 경로 |
| `dispatch` | Phase 4 의 에이전트 호출 |
| `tier_map` | spec 의 `tier` → 실제 모델 |
| `skill_call` | `REQUIRED SUB-SKILL` 호출 문법 |
| `superpowers_roots` | Phase 1 의 미설치 안내 |
| `guard` | 읽기 전용 경계의 **실제** 강제 수준 |
| `worktree` | Phase 4 의 격리 가능 여부 |

판정에 실패하면(exit 3) 사용자에게 어떤 하네스인지 묻고 진행한다. **추측하지 않는다** —
잘못된 어댑터를 고르면 dispatch 가 통째로 실패한다.

## Phase −1 — 재개 판정 (Phase 0보다 먼저)

`Bash: python3 ${skill_dir}/scripts/resume-check.py`
...
- exit 11 → **사람 판단.** 브리핑을 제시하고 **멈춘다.** 사용자가 셋 중 하나를 고른다:
  ...
  - **재판정** — Phase 2 로 돌아가 6축을 다시 판정한 뒤
    `Bash: python3 ${skill_dir}/scripts/checkpoint.py --replan --level <새 레벨>`
    으로 승인·진행·게이트·리뷰 루프를 초기화하고 Phase 3(HarnessSpec 재작성)부터 다시 간다.
  - **폐기** — `Bash: python3 ${skill_dir}/scripts/checkpoint.py --discard`
...
> **다른 하네스에서 재개하는 경우.** `resume-check.py` 는 state 에 기록된 하네스와 지금 하네스가
> 다르면 불일치로 보고 exit 11(사람 판단)로 넘긴다 — dispatch 문법·모델·가드 강제 수준이
> 전부 달라지기 때문이다. 이어받으려면 새 하네스의 어댑터 기준으로 다시 승인받는다.
...
4. 정규화가 끝나면 기록한다:
   `Bash: python3 ${skill_dir}/scripts/checkpoint.py --phase 0 --goal "<정규화한 goal>"`

## Phase 1 — 결정론적 게이트 탐지

`Bash: bash ${skill_dir}/scripts/init-workspace.sh`
...
- exit 4 → superpowers 미설치. **여기서 멈춘다.** 하네스는 H0 조차
  ...
  1. 사용자에게 설치 여부를 묻는다.
  2. 승인하면 **어댑터의 `superpowers_roots` 가 알려주는 그 하네스의 설치 명령을 제시한다.**
     (`init-workspace.sh` 의 exit 4 출력에 이미 들어 있다.) 설치는 사용자가 직접 한다 —
     슬래시 명령·플러그인 등록은 스킬이 대신 실행할 수 없다.
...
4. 판정이 끝나면 기록한다:
   `Bash: python3 ${skill_dir}/scripts/checkpoint.py --phase 2 --level <판정> --next "<다음 행동>"`
...
2. `Bash: python3 ${skill_dir}/scripts/validate-spec.py _workspace/harness/spec.yaml --gates _workspace/harness/gates.tsv`
...
4. **한 화면 요약**을 제시한다: 레벨 + 근거 / 에이전트와 tier(어댑터가 매핑한 실제 모델 병기) /
   게이트 / 리뷰 루프 상한 /
   Human Gate 여부와 사유 / 병렬도 / 추적 링크. validator 경고가 있으면 함께 보인다.
5. **승인을 받는다.** 승인되면 상태를 `Triage` → `Todo` 로 올리고, 승인 직후 기록한다:
   `Bash: python3 ${skill_dir}/scripts/checkpoint.py --phase 3 --approved --agents <spec의 agents id 쉼표 목록> --artifact spec=_workspace/harness/spec.yaml`
...
`Bash: bash ${skill_dir}/scripts/gate-summary.sh <tier>` 출력을 코멘트 1건으로 남긴다
...
1. `Bash: bash ${skill_dir}/scripts/run-gates.sh final`
...
3. `human_gate.required` 면 증거(게이트 로그 경로 + diff 통계 + 롤백 절차)를 제시하고 멈춘다.
   ...
   `Bash: python3 ${skill_dir}/scripts/checkpoint.py --human-gate-passed`
...
5. 종료를 기록한다:
   `Bash: python3 ${skill_dir}/scripts/checkpoint.py --phase done`
...
- **컨텍스트는 경로로 전달한다**: 보고서 본문·세션 히스토리를 dispatch 프롬프트에 붙여넣지 않는다. dispatch 시 spec 의 `tier` 를 어댑터 `tier_map` 으로 옮겨 **모델을 항상 명시한다** (생략하면 세션의 가장 비싼 모델을 상속).
- **어댑터를 건너뛰지 않는다**: 하네스 고유 문법(도구 이름·모델 이름·훅 경로)을 이 문서나 references 에 직접 쓰지 않는다. 새로 필요해지면 어댑터 키로 올린다.
...
- **이식성**: `bash install.sh <대상> [--harness claude|codex|opencode]` 로 배치한다. 코어(이 문서·references·schemas·scripts·roles)는 하네스와 무관하게 한 벌이고, 하네스별 차이는 어댑터 트리와 `references/adapters/*.md` 에만 있다.
