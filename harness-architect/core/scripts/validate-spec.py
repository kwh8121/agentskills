...
# 게이트 tier (검증 단계). detect-stack.sh · run-gates.sh 와 같은 집합이어야 한다.
TIERS = {"fast", "feature", "final"}

# 에이전트 tier (모델 등급). 게이트 tier 와 이름이 겹치지 않게 구분해서 읽는다.
# 실제 모델 매핑은 references/adapters/<하네스>.md 의 tier_map 이 정한다.
TIERS_MODEL = {"fast", "standard", "deep"}
...
def check_agents(spec, r):
    ...
    for i, a in enumerate(agents):
        ...
        # tier 가 하네스 중립 표기이고, model 은 하네스 고유 표기다. 둘 중 하나는 있어야 한다 —
        # 없으면 dispatch 가 세션의 가장 비싼 모델을 상속한다.
        tier, model = a.get("tier"), a.get("model")
        if tier and tier not in TIERS_MODEL:
            r.error("E-AGENT-TIER", f"agents[{i}]({aid})",
                    f"tier '{tier}' 는 없다. 가능: {sorted(TIERS_MODEL)}")
        if not tier and not model:
            r.error("E-AGENT-MODEL", f"agents[{i}]({aid})",
                    "tier 도 model 도 없다. dispatch 시 생략하면 세션의 가장 비싼 모델을 상속한다 "
                    "(하네스 중립 표기는 tier, 하네스 고유 표기는 model — "
                    "매핑은 references/adapters/<하네스>.md 의 tier_map)")
        if not tier and model:
            r.warn("W-AGENT-TIER", f"agents[{i}]({aid})",
                   f"model '{model}' 만 있다. 다른 하네스에서 재개할 수 없는 spec 이 된다 — "
                   f"tier(fast|standard|deep)를 함께 쓰는 것을 권한다")
