...
from checkpoint import (                                     # noqa: E402
    detect_harness, repo_fingerprint, resolve, spec_digest, validate_state,
    SCHEMA_VERSION,
)
...
def drift(recorded, current, state):
    ...
    # 다른 하네스에서 재개하는가. 하네스가 바뀌면 dispatch 문법·모델·가드 강제 수준이
    # 전부 달라지므로(references/adapters/*.md) 자동으로 이어받지 않고 사람에게 넘긴다.
    # 기록이 없는 state(구버전)와 판정 불가는 불일치로 치지 않는다 — 모르는 것을
    # 불일치로 만들면 정상 재개가 막힌다.
    was_h = state.get("harness")
    now_h = detect_harness()
    if was_h and now_h and was_h != now_h:
        out.append(f"하네스 {was_h} → {now_h} (승인·dispatch 규약이 다르다)")
...
def render(state, marks, path):
    ...
    print(f"[재개] {state.get('level') or '?'} · Phase {state.get('phase')} · "
          ...
          f"· 하네스 {state.get('harness') or '기록 없음'}")
