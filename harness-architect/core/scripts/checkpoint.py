...
def detect_harness():
    """지금 어떤 하네스에서 돌고 있는지. 판정은 detect-harness.sh 한 곳에서만 한다.

    판정하지 못하면 None 을 남긴다 — 모르는 것을 적어 두면 재개할 때 잘못된 불일치를
    만들어 낸다. schema_version 은 올리지 않는다: 이 필드는 선택 필드이고, 이전 세션이
    만든 state(harness 없음)는 그대로 재개 가능해야 한다.
    """
    script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "detect-harness.sh")
    try:
        out = subprocess.run(["bash", script, "--field", "harness"],
...
def validate_state(state):
    ...
    if state.get("harness") is not None and not isinstance(state.get("harness"), str):
        errs.append("harness 가 문자열이 아닙니다")
